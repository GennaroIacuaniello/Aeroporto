...
BEGIN
	...
	IF NEW.is_deleted = false THEN
		...
		IF EXISTS(SELECT * FROM ADMIN A
				WHERE A.id_admin <> NEW.id_admin AND A.is_deleted = false
				AND A.username = NEW.username) THEN 
...
BEGIN
	...
	IF NEW.is_deleted = false THEN
		...
		IF EXISTS(SELECT * FROM CUSTOMER C
				WHERE C.id_customer <> NEW.id_customer AND C.is_deleted = false
				AND C.username = NEW.username) THEN 
...
BEGIN
	...
	IF OLD.is_deleted = true THEN
		...
		RAISE EXCEPTION 'L''account dell''utente % è cancellato, non può essere modificato!',
				OLD.id_admin;
...
CREATE TYPE FlightStatus AS ENUM ('PROGRAMMED', 'CANCELLED', 'ABOUT_TO_DEPART', 'DEPARTED',
				  'DELAYED', 'LANDED', 'ABOUT_TO_ARRIVE');
...
CREATE TABLE Flight (
	...
	flight_delay Minutes NOT NULL DEFAULT 0,
	...
	--l'aeroporto di Napoli ha 20 gate:
	CONSTRAINT correctness_of_id_gate CHECK(id_gate IS NULL OR id_gate BETWEEN 1 AND 20)
...
BEGIN
	...
	IF NEW.flight_status <> 'PROGRAMMED' AND NEW.flight_status <> 'CANCELLED' THEN
		...
		IF OLD.id_flight <> NEW.id_flight THEN
			...
			RAISE EXCEPTION 'Il volo % non è in stato ''programmato'' o ''cancellato'' 
					non può cambiare id!', OLD.id_flight;
...
BEGIN
	...
	IF OLD.flight_type = false THEN 
		...
		IF NEW.flight_status = 'DEPARTED' OR NEW.flight_status = 'DELAYED'
		   OR NEW.flight_status = 'ABOUT_TO_ARRIVE' OR NEW.flight_status = 'LANDED' THEN

			IF OLD.departure_time <> NEW.departure_time THEN

				RAISE EXCEPTION 'Il volo % è già partito, non si può modificare 
						il suo orario di partenza!', OLD.id_flight;
...
BEGIN
	...
	IF OLD.flight_status = 'LANDED' AND NEW.flight_status = 'LANDED' THEN
		...
		IF OLD.arrival_time <> NEW.arrival_time THEN
			...
			RAISE EXCEPTION 'Il volo % è già atterrato, non si può modificare
					il suo orario di arrivo!', OLD.id_flight;
	...
	IF OLD.flight_type = true THEN 
		...
		IF NEW.flight_status = 'DEPARTED' OR NEW.flight_status = 'LANDED' THEN
			...
			IF OLD.flight_delay <> NEW.flight_delay THEN
				...
				RAISE EXCEPTION 'Il volo da Napoli % è già partito, non si può modificare
						il ritardo di partenza!', OLD.id_flight;
	...
	IF OLD.flight_type = false THEN 
		...
		IF NEW.flight_status = 'LANDED' THEN
			...
			IF OLD.flight_delay <> NEW.flight_delay THEN
				...
				RAISE EXCEPTION 'Il volo per Napoli % è già atterrato,
						non si può modificare il ritardo di arrivo!', OLD.id_flight;
...
BEGIN
	...
	IF NEW.flight_type = false THEN
		...
		IF NEW.flight_status = 'ABOUT_TO_DEPART' THEN
			...
			RAISE EXCEPTION 'Il volo % è verso Napoli, non può avere
					stato ''in partenza''', NEW.id_flight;
...
BEGIN
	...
	IF NEW.flight_type = true THEN
		...
		IF NEW.flight_status = 'ABOUT_TO_ARRIVE' THEN
			...
			RAISE EXCEPTION 'Il volo % è da Napoli, non può avere stato ''in arrivo''',
					NEW.id_flight;
...
CREATE TABLE Booking (
	...
	CONSTRAINT buyer_FK FOREIGN KEY(buyer) REFERENCES Customer(id_customer) ON DELETE CASCADE
										ON UPDATE CASCADE,
	CONSTRAINT id_flight_FK FOREIGN KEY(id_flight) REFERENCES Flight(id_flight) ON DELETE CASCADE
										    ON UPDATE CASCADE
...
BEGIN
	...
	IF OLD.company_name <> NEW.company_name OR OLD.max_seats <> NEW.max_seats
	   OR OLD.destination_or_origin <> NEW.destination_or_origin
	   OR OLD.flight_type <> NEW.flight_type THEN

		IF EXISTS(SELECT * FROM BOOKING B
				  WHERE B.id_flight = OLD.id_flight) THEN

			RAISE EXCEPTION 'Il volo % ha già almeno una prenotazione associata,
					non si possono modificare i suoi: nome compagnia, posti massimi,
					destinazione o origine, tipo!', OLD.id_flight;
...
BEGIN
	...
	IF OLD.id_booking <> NEW.id_booking THEN
		...
		IF associated_flight.flight_status <> 'PROGRAMMED'
		   AND associated_flight.flight_status <> 'CANCELLED' THEN 

			RAISE EXCEPTION 'Il volo %, associato alla prenotazione %,
					non è in stato ''programmato'' o ''cancellato'',
					non si può modificare l''id della prenotazione!', 
...
--TRIGGER SE UN VOLO è ABOUT_TO_DEPART, DEPARTED, ABOUT_TO_ARRIVE o  LANDED,
--LE PRENOTAZIONI NON POSSONO PIÙ ESSERE MODIFICATE (NEMMENO CANCELLATE)
...
BEGIN
	...
	IF associated_flight.flight_status = 'ABOUT_TO_DEPART'
	   OR associated_flight.flight_status = 'DEPARTED'
	   OR associated_flight.flight_status = 'ABOUT_TO_ARRIVE'
	   OR associated_flight.flight_status = 'LANDED' THEN

		RAISE EXCEPTION 'Il volo % è in partenza/partito/sta per atterrare/atterrato,
				non si può modificare la prenotazione!', NEW.id_flight;
...
CREATE TABLE Ticket (
	...
	CONSTRAINT booking_FK FOREIGN KEY(id_booking) REFERENCES Booking(id_booking) ON DELETE CASCADE
										     ON UPDATE CASCADE,
	CONSTRAINT passenger_FK FOREIGN KEY(id_passenger) REFERENCES Passenger(SSN) ON DELETE RESTRICT
										    ON UPDATE CASCADE,
	--ON DELETE RESTRICT, quindi un passeggero non può essere cancellato
	--finchè ha ancora almeno un biglietto ad esso associato
	CONSTRAINT id_flight_FK FOREIGN KEY(id_flight) REFERENCES Flight(id_flight) ON DELETE CASCADE
									 	    ON UPDATE CASCADE,
...
BEGIN
	...
	IF OLD.id_flight <> NEW.id_flight THEN
		...
		IF EXISTS(SELECT * FROM TICKET T
			...
			RAISE EXCEPTION 'La prenotazione % ha almeno un biglietto associato che si
					riferisce ad un altro volo, non è possibile renderla una
					prenotazione per il volo %!', OLD.id_booking, NEW.id_flight;
...
--Qui non serve il BEFORE INSERT, perchè non posso avere inserito TICKET
--per una prenotazione che ancora non c'era
...
BEGIN
	...
	IF OLD.id_flight <> NEW.id_flight THEN
		...
		IF NEW.id_flight <> associated_booking.id_flight THEN
			...
			RAISE EXCEPTION 'Il biglietto con numero % è associato ad una prenotazione 
					per un altro volo, non è possibile renderlo un biglietto
					per il volo %!', OLD.ticket_number, NEW.id_flight;
	...
	IF OLD.ticket_number <> NEW.ticket_number THEN
		...
		IF associated_flight.flight_status <> 'PROGRAMMED'
		   AND associated_flight.flight_status <> 'CANCELLED' THEN 
			...
			RAISE EXCEPTION 'Il volo %, associato al biglietto con numero %, non
					è in stato ''programmato'' o ''cancellato'', non
					si può modificare il numero del biglietto!',
...
BEGIN
	...
	IF NOT EXISTS(SELECT * FROM TICKET T
				  ...
				  --non devo fare il controllo su T.ticket_number <> OLD.ticket_number
				  --perchè il trigger è AFTER DELETE,
...
--TRIGGER I PASSEGERI POSSONO AVERE QUALUNQUE DATO (TRANNE L'SSN PK) A NULL,
--MA QUESTO SOLO SE NESSUNA LORO PRENOTAZIONE È 'CONFFIRMED', 
	...
	--solo se effettivamente qualcosa è a null faccio il controllo
		...
		FOR selected_ticket IN (SELECT * FROM TICKET T
			...
			IF associated_booking.booking_status = 'CONFIRMED' THEN
				...
				RAISE EXCEPTION 'Dati mancanti per il passeggero il cui biglietto ha
...
--TRIGGER PRIMA DI METTERE UN PRENOTAZIONE A CONFERMATA, CONTROLLO CHE
--TUTTI I PASSEGGERI AD ESSA ASSOCIATA NON ABBIANO DATI NULL
...
BEGIN
	...
	IF OLD.booking_status <> 'CONFIRMED' AND NEW.booking_status = 'CONFIRMED' THEN
		...
		FOR selected_ticket IN (SELECT * FROM TICKET T
			...
			IF associated_passenger.first_name IS NULL
			  OR associated_passenger.last_name IS NULL
			  OR associated_passenger.birth_date IS NULL THEN

				RAISE EXCEPTION 'Dati mancanti per il passeggero il cui biglietto
						ha numero %, la prenotazione % non può essere confermata!', 
...
--TRIGGER SE FACCIO IL CHECK-IN (OSSIA SE CHECK-IN = TRUE), ALLORA SE POSTO È NULL,
--ASSEGNO AL TICKET IL PRIMO POSTO LIBERO E QUESTO TRIGGER È BEFORE INSERT OR UPDATE
--OF CHECKED_IN, SEAT; COSÌ NON PUÒ MAI ACCADERE CHE UN TICKET CON CHECK-IN TRUE NON ABBIA POSTO)
...
BEGIN
	...
	IF NEW.checked_in = true AND NEW.seat IS NULL THEN
		...
		FOR selected_seat IN (SELECT T.seat FROM TICKET T
				      ...
				      AND T.id_flight = NEW.id_flight AND 
						(SELECT B.booking_status FROM BOOKING B
						 WHERE B.id_booking = T.id_booking) <> 'CANCELLED'
		...
		--se non c'era un 'buco' tra i posti disponibili, assegno il posto fuori dal loop
		...
		--controllo di non aver sforato il posto massimo
		IF NEW.seat > associated_flight.max_seats THEN		
			
			RAISE EXCEPTION 'Errore nell''assegnazione del posto per il passeggero
					con ticket_number: %, per il volo: %', NEW.ticket_number,
					NEW.id_flight;
...
--TRIGGER SE ALMENO UN VOLO DI UN PASSEGGERO NON È IN STATO PROGRAMMED O CANCELLED,
--I DATI DEL PASSEGGERO NON POSSONO PIÙ ESSERE MODIFICATI
...
BEGIN
	...
	IF NEW.first_name <> OLD.first_name OR NEW.last_name <> OLD.last_name
	   OR NEW.birth_date <> OLD.birth_date OR NEW.SSN <> OLD.SSN THEN
		...
		FOR selected_ticket IN (SELECT * FROM TICKET T
			...
			IF associated_flight.flight_status <> 'PROGRAMMED'
			   AND associated_flight.flight_status <> 'CANCELLED' THEN
				...
				RAISE EXCEPTION 'Il volo % è in stato %, non si possono modificare
						i dati del passeggero con SSN %!', 
						associated_flight.id_flight, associated_flight.flight_status,
						NEW.SSN;
...
BEGIN
	...
	IF associated_flight.flight_status = 'ABOUT_TO_DEPART' THEN
		...
		IF OLD.ticket_number <> NEW.ticket_number OR OLD.id_booking <> NEW.id_booking
		   OR OLD.id_passenger <> NEW.id_passenger OR OLD.id_flight <> NEW.id_flight THEN
		
			RAISE EXCEPTION 'Il volo % è in stato ''in partenza'', per il biglietto con
					ticket_number % si può solo modificare il posto o fare il check-in!',
...
--TRIGGER SE UN VOLO È DEPARTED o ABOUT_TO_ARRIVE O LANDED,
--I DATI DI UN TICKET NON POSSONO PIÙ ESSERE MODIFICATI
...
BEGIN
	...
	IF associated_flight.flight_status = 'DEPARTED' OR associated_flight.flight_status = 'ABOUT_TO_ARRIVE'
	   OR associated_flight.flight_status = 'LANDED' THEN

		RAISE EXCEPTION 'Il volo % è in stato %, non si possono modificare
				i dati del biglietto con ticket_number %!',
...
BEGIN
	...
	IF associated_booking.booking_status <> 'CANCELLED' AND NEW.seat IS NOT NULL THEN
		...
		IF EXISTS(SELECT * FROM TICKET T
		  	  WHERE T.ticket_number <> NEW.ticket_number 
			  AND T.id_flight = NEW.id_flight AND T.seat = NEW.seat 
...
--TRIGGER CHE AGGIORNA I FREE_SEATS DI UN VOLO ALL'INSERIMENTO DI UN BIGLIETTO
--(DI UNA PRENOTAZIONE NON CANCELLATA) PER QUEL VOLO
...
--TRIGGER CHE AGGIORNA I FREE_SEATS DI UN VOLO ALLA CANCELLAZIONE DI UN BIGLIETTO
--(DI UNA PRENOTAZIONE NON CANCELLATA) PER QUEL VOLO
...
BEGIN
	...
	IF NEW.checked_in = true THEN
		...
		IF associated_booking.booking_status <> 'CONFIRMED' THEN
			...
			RAISE EXCEPTION 'La prenotazione % non è confermata,
					non si può fare il check-in!', NEW.id_booking;
...
--TRIGGER SOLO PER I VOLI ABOUT_TO_DEPART, DEPARTED, ABOUT_TO_ARRIVE
--o LANDED UN BIGLIETTO PUÒ ESSERE CHECKED_IN
...
BEGIN
	...
	IF NEW.checked_in = true THEN
		...
		IF associated_flight.flight_status <> 'ABOUT_TO_DEPART'
		   AND associated_flight.flight_status <> 'DEPARTED'
		   AND associated_flight.flight_status <> 'ABOUT_TO_ARRIVE'
		   AND associated_flight.flight_status <> 'LANDED' THEN

			RAISE EXCEPTION 'Il volo % non è in partenza/partito/sta per atterrare/atterrato,
					non si può essere CHECKED-in!', NEW.id_flight;
...
BEGIN
	...
	IF OLD.checked_in = true AND NEW.checked_in = false THEN
		...
		IF associated_flight.flight_status = 'DEPARTED'
		   OR associated_flight.flight_status = 'ABOUT_TO_ARRIVE'
		   OR associated_flight.flight_status = 'LANDED' THEN

			RAISE EXCEPTION 'Il volo % è già partito, non si può annullare il check-in!',
					NEW.id_flight;
...
CREATE TABLE Luggage (
	...
	CONSTRAINT ticket_FK FOREIGN KEY(id_ticket) REFERENCES Ticket(ticket_number) ON DELETE CASCADE
										     ON UPDATE CASCADE,
	CONSTRAINT correctness_of_id_luggage_after_check_in_minimal_length
		CHECK( id_luggage_after_check_in IS NULL OR LENGTH(id_luggage_after_check_in) > 13)
...
BEGIN
	...
	IF NEW.luggage_type = 'CARRY_ON' THEN
		...
		IF EXISTS(SELECT * FROM LUGGAGE L
			...
			RAISE EXCEPTION 'Per il biglietto con numero % c''è già un bagaglio a mano,
					non se ne possono inserire altri!', associated_ticket.ticket_number;
...
--TRIGGER SE PER UN BIGLIETTO C'è GIà UN BAGAGLIO A MANO, NON SI PUò MODIFICARE
--IL TIPO DI UN ALTRO BAGAGLIO PER FAR SI CHE CE NE SIANO DUE (BEFORE UPDATE)
...
BEGIN
	...
	IF OLD.luggage_type <> 'CARRY_ON' AND NEW.luggage_type = 'CARRY_ON' THEN
		...
		IF EXISTS(SELECT * FROM LUGGAGE L
			  WHERE L.id_luggage <> OLD.id_luggage AND L.id_ticket = OLD.id_ticket
			  AND L.luggage_type = 'CARRY_ON') THEN

			RAISE EXCEPTION 'Per il biglietto con numero % c''è già un bagaglio a mano,
					non se ne possono avere altri!', associated_ticket.ticket_number;
	...
	IF (OLD.id_luggage <> NEW.id_luggage) OR (OLD.id_ticket <> NEW.id_ticket) THEN
		...
		IF associated_flight.flight_status <> 'PROGRAMMED'
		   AND associated_flight.flight_status <> 'CANCELLED' THEN 
			...
			RAISE EXCEPTION 'Il volo %, associato al bagaglio %, non è in
					stato ''programmato'' o ''cancellato'', non si può modificare
					l''id del bagaglio o il suo biglietto associato!',
...
BEGIN
	...
	IF (OLD.id_luggage_after_check_in IS NOT NULL)
	   AND (OLD.id_luggage_after_check_in <> NEW.id_luggage_after_check_in) THEN

		RAISE EXCEPTION 'Per il bagaglio % è già stato generato l''id after check-in,
				non si può modificarlo!', OLD.id_luggage;
...
--TRIGGER PER I BAGAGLI, SE LA PRENOTAZIONE DEL BIGLIETTO ASSOCIATO NON è CONFIRMED,
--IL TIPO PUò ESSERE A NULL, MA SE è CONFIRMED NO
...
BEGIN
	...
	IF associated_booking.booking_status = 'CONFIRMED' THEN
		...
		IF NEW.luggage_type IS NULL THEN
			...
			RAISE EXCEPTION 'Dati mancanti per il bagaglio % del passeggero il cui
					biglietto ha numero %',
					NEW.id_luggage, associated_ticket.ticket_number;
...
BEGIN
	...
	IF associated_ticket.checked_in = false THEN
		...
		IF NEW.luggage_status <> 'BOOKED' THEN
			...
			RAISE EXCEPTION 'Il biglietto con ticket number % non ha ancora fatto il
					check_in, il bagaglio % non può avere stato divero da ''prenotato''!',
					associated_ticket.ticket_number, NEW.id_luggage;
...
--TRIGGER QUANDO PER UN BIGLIETTO VIENE FATTO IL CHECK-IN, VIENE GENERATO
--L'id_luggage_after_check_in PER TUTTI I SUOI BAGAGLI (VIENE GENERATO COME 
--ticket_number concatenato un intero da 0 a numero di bagagli del passeggero - 1)
...
BEGIN
	...
	IF associated_ticket.checked_in = true THEN
		...
		IF NEW.id_luggage_after_check_in IS NULL THEN
			...
			RAISE EXCEPTION 'Il biglietto con ticket number % ha efettuato il check_in,
					il suo bagaglio non può non avere id_luggage_after_check_in!', 
...
--TRIGGER IL LUGGAGE_STATUS (DEI BAGAGLI DELLE PRENOTAZIONI NON CANCELLATE)
--NON PUÒ ESSERE BOOKED SE IL VOLO È DEPARTED, ABOUTTOARRIVE O LANDED
		...
		IF associated_booking.booking_status <> 'CANCELLED' THEN
			...
			IF associated_flight.flight_status = 'DEPARTED'
			   ...
			   OR associated_flight.flight_status = 'LANDED' THEN
				...
				RAISE EXCEPTION 'Il volo % del passeggero con ticket number % è
						già partito, il bagaglio % non può avere
						stato ''prenotato''!', 
						associated_ticket.id_flight, associated_ticket.ticket_number,
						NEW.id_luggage;
...
BEGIN
	...
	IF NEW.luggage_status = 'WITHDRAWABLE' THEN
		...
		IF associated_flight.flight_status <> 'LANDED' THEN
			...
			RAISE EXCEPTION 'Il volo % del passeggero con ticket number % non è ancora atterrato,
					il bagaglio % non può avere stato ''ritirabile''!',
					associated_ticket.id_flight, associated_ticket.ticket_number,
					NEW.id_luggage;
...
BEGIN
	...
	IF NEW.luggage_status = 'LOST' THEN
		...
		IF associated_flight.flight_status <> 'LANDED' THEN
			...
			RAISE EXCEPTION 'Il volo % del passeggero con ticket number % non è
					ancora atterrato, il bagaglio % non può avere stato ''smarrito''!',
					associated_ticket.id_flight, associated_ticket.ticket_number,
					NEW.id_luggage;
...
BEGIN
	...
	IF NEW.luggage_status = 'LOADED' THEN
		...
		IF associated_flight.flight_status = 'PROGRAMMED'
		   OR associated_flight.flight_status = 'CANCELLED'
		   OR associated_flight.flight_status = 'LANDED' THEN
			
			RAISE EXCEPTION 'Il volo % del passeggero con ticket number % è
					progammato/cancellato/atterrato, il bagaglio %
					non può avere stato ''caricato''!',
					associated_ticket.id_flight, associated_ticket.ticket_number,
					NEW.id_luggage;
...
BEGIN
	...
	IF NEW.luggage_status <> 'BOOKED' THEN
		...
		IF associated_booking.booking_status = 'CANCELLED' THEN
			...
			RAISE EXCEPTION 'La prenotazione % del passeggero con ticket number %
					è cancellata, il bagaglio % non può avere stato divero
					da ''prenotato''!',
					associated_ticket.id_booking, associated_ticket.ticket_number,
					NEW.id_luggage;
...
BEGIN
	...
	IF NEW.luggage_status <> OLD.luggage_status THEN
		...
		IF NEW.luggage_status = 'LOADED' AND OLD.luggage_status <> 'BOOKED' THEN
			...
			RAISE EXCEPTION 'Il bagaglio % non era in stato ''prenotato'', non
					può diventare ''caricato''!', OLD.id_luggage;
...
BEGIN
	...
	IF NEW.luggage_status <> OLD.luggage_status THEN
		...
		IF NEW.luggage_status = 'WITHDRAWABLE' AND (OLD.luggage_status <> 'LOADED'
		   AND OLD.luggage_status <> 'LOST' ) THEN
			
			RAISE EXCEPTION 'Il bagaglio % non era in stato ''caricato'' o ''disperso'', non
					può diventare ''ritirabile''!', OLD.id_luggage;
...
BEGIN
	...
	IF NEW.luggage_status <> OLD.luggage_status THEN
		...
		IF NEW.luggage_status = 'LOST' AND OLD.luggage_status <> 'WITHDRAWABLE' THEN
			...
			RAISE EXCEPTION 'Il bagaglio % non era in stato ''ritirabile'', non può
					diventare ''smarrito''!', OLD.id_luggage;
...
--TRIGGER QUANDO UN VOLO DEPARTING PARTE, TUTTI I LUGGAGE_STATUS DEI BAGAGLI DEI SUOI
--BIGLIETTI CON CHECKED_IN A TRUE VENGONO MESSI A LOADED
	...
	IF OLD.flight_type = true AND NEW.flight_type = true THEN
		...
		IF OLD.flight_status <> 'DEPARTED' AND NEW.flight_status = 'DEPARTED' THEN
			...
			IF OLD.flight_status <> 'ABOUT_TO_DEPART' THEN
				...
				RAISE EXCEPTION 'Il volo da Napoli % non era in stato ''in partenza'', non
						può diventare ''partito''!', OLD.id_flight;

			END IF;

			FOR selected_booking IN (SELECT * FROM BOOKING B
						 WHERE B.id_flight = NEW.id_flight
						 AND B.booking_status <> 'CANCELLED') LOOP

				--non devo controllare la prenotazione non sia PENDING, 
				--perchè tanto il volo era per forza ABOUT_TO_DEPART, e quindi
				--le sue prenotazioni già non potevano essere PENDING
				FOR selected_ticket IN (SELECT * FROM TICKET T
							WHERE T.id_booking = selected_booking.id_booking
							AND T.checked_in = true) LOOP
...
--TRIGGER QUANDO UN VOLO ATTERRA, TUTTI I LUGGAGE_STATUS DEI BAGAGLI
--DEI SUOI BIGLIETTI CHECKED_IN VENGONO MESSI A WITHDRAWABLE
...
BEGIN
	...
	IF OLD.flight_type = true AND NEW.flight_type = true THEN
		...
		IF OLD.flight_status <> 'LANDED' AND NEW.flight_status = 'LANDED' THEN 
			...
			--questo if serve perché solo un volo DEPARTED o ABOUT_TO_ARRIVE
			--può essere impostato a DEPARTED
			IF OLD.flight_status <> 'DEPARTED' AND OLD.flight_status <> 'ABOUT_TO_ARRIVE' THEN

				RAISE EXCEPTION 'Il volo % non era in stato ''partito'' o ''sta per
						arrivare'', non può diventare ''atterrato''!', OLD.id_flight;
			...
			FOR selected_booking IN (SELECT * FROM BOOKING B
				...
				--non devo controllare la prenotazione non sia PENDING, 
				--perchè tanto il volo era già partito, e quindi le sue
				--prenotazioni già non potevano essere PENDING
				FOR selected_ticket IN (SELECT * FROM TICKET T
							WHERE T.id_booking = selected_booking.id_booking
							AND T.checked_in = true) LOOP
...
--TRIGGER UN VOLO PUÒ ESSERE ABOUT_TO_DEPART O DELAYED O DEPARTED O ABOUT_TO_ARRIVE O
--LANDED SOLO SE FLIGHT.DEPARTURE_TIME::DATE <= DATA_CORRENTE (IMPLEMENTATO CON 
--IF (ABOUT_TO_DEPART O DEPARTED O LANDED) THEN IF DATA > DATA_CORRENTE THEN RAISE EXCEPTION)
...
BEGIN
	...
	IF NEW.flight_status = 'ABOUT_TO_DEPART' OR NEW.flight_status = 'DELAYED' OR THEN
		...
		--posso aprire i check-in (e quindi mettere ad ABOUT_TO_DEPART)
		--al più il giorno prima della partenza del volo e quindi non
		--posso aprirli solo se NEW.departure_time::date non è nè oggi nè domani
		--(e stessa cosa per il ritardo di un volo)

			RAISE EXCEPTION 'Non possono ancora essere aperti i check-in per il volo %!
					La sua data di partenza è % con ritardo di % minuti!',
					NEW.id_flight, NEW.departure_time::date, NEW.flight_delay;

		END IF;
	
	ELSIF  NEW.flight_status = 'DEPARTED' OR NEW.flight_status = 'ABOUT_TO_ARRIVE'
	       OR NEW.flight_status = 'LANDED' THEN 

		IF (NEW.departure_time::date) > CURRENT_DATE THEN
		--un volo deve invece partire almeno nella data stabilita,
		--al più partirà dopo per ritardi

			RAISE EXCEPTION 'Il volo % non può ancora partire! La sua data
					di partenza è % con ritardo di % minuti!',
					NEW.id_flight, NEW.departure_time::date, NEW.flight_delay;
...
BEGIN
	...
	IF NEW.flight_status = 'ABOUT_TO_DEPART' THEN
		...
		IF OLD.flight_type = false THEN
			...
			RAISE EXCEPTION 'Il volo % è verso Napoli, non può avere
					stato ''in partenza''', OLD.id_flight;

		END IF;

		IF OLD.flight_status <> 'PROGRAMMED' THEN

			RAISE EXCEPTION 'Il volo % non era in stato ''programmato'', non
					può diventare ''in partenza''!', OLD.id_flight;
...
BEGIN
	...
	IF NEW.flight_status = 'DEPARTED' THEN
		...
		IF OLD.flight_status <> 'ABOUT_TO_DEPART' THEN
			...
			RAISE EXCEPTION 'Il volo % non era in stato ''in partenza'', non
					può diventare ''partito''!', OLD.id_flight;
...
--TRIGGER SOLO UN VOLO PROGRAMMED O ABOUTTODEPART O DEPARTED O ABOUTTOARRIVE
--Può ESSERE AGGIORNATO A DELAYED
...
BEGIN
	...
	IF NEW.flight_status = 'DELAYED' THEN
		...
		IF OLD.flight_status <> 'PROGRAMMED' AND OLD.flight_status <> 'ABOUT_TO_DEPART'
		   AND OLD.flight_status <> 'DEPARTED' AND OLD.flight_status <> 'ABOUT_TO_ARRIVE' THEN

			RAISE EXCEPTION 'Il volo % non era in stato ''programmato'' o ''in partenza'', non
					può diventare ''in ritardo''!', OLD.id_flight;
	...
	IF OLD.flight_type = true AND NEW.flight_type = true THEN
		...
		IF NEW.flight_status = 'LANDED' THEN
			...
			IF OLD.flight_status <> 'DEPARTED' THEN
				...
				RAISE EXCEPTION 'Il volo % non era in stato ''partito'', non
						può diventare ''atterrato''!', OLD.id_flight;
	...
	IF OLD.flight_type = false AND NEW.flight_type = false THEN
		...
		IF NEW.flight_status = 'ABOUT_TO_ARRIVE' THEN
			...
			IF OLD.flight_status <> 'DEPARTED' THEN
				...
				RAISE EXCEPTION 'Il volo % non era in stato ''partito'', non può
						diventare ''in arrivo''!', OLD.id_flight;
	...
	IF OLD.flight_type = false AND NEW.flight_type = false THEN
		...
		IF NEW.flight_status = 'LANDED' THEN
			...
			IF OLD.flight_status <> 'DEPARTED' AND OLD.flight_status <> 'ABOUT_TO_ARRIVE' THEN
				...
				RAISE EXCEPTION 'Il volo % non era in stato ''partito'' o ''in arrivo'', non
						può diventare ''atterrato''!', OLD.id_flight;
...
--TRIGGER QUANDO IL FLIGHT STATUS DI UN VOLO DEPARTING DIVENTA 'ABOUT_TO_DEPART',
--LE PRENOTAZIONI 'PENDING' DIVENTANO 'CANCELLED'

CREATE OR REPLACE FUNCTION actual_func_change_booking_status_when_dep_aToDep(input_old_id_flight VARCHAR(15),
			   input_old_flight_type BOOLEAN, input_old_flight_status FlightStatus,
			   input_new_flight_type BOOLEAN, input_new_flight_status FlightStatus)
RETURNS INTEGER
AS $$
DECLARE
...
BEGIN
	...
	IF input_old_flight_type = true AND input_new_flight_type = true THEN
		...
		IF input_old_flight_status <> 'PROGRAMMED' THEN
			...
			RAISE EXCEPTION 'Il volo da Napoli % non era in stato ''programmato'', non
					può diventare ''in partenza''!', OLD.id_flight;

		END IF;

		IF input_old_flight_status <> 'ABOUT_TO_DEPART'
		   AND input_new_flight_status = 'ABOUT_TO_DEPART' THEN
		
			FOR selected_booking IN (SELECT * FROM BOOKING B
						 WHERE B.id_flight = input_old_id_flight
						 AND B.booking_status = 'PENDING') LOOP
...
BEGIN
	...
	SELECT actual_func_change_booking_status_when_dep_aToDep(OLD.id_flight,
	       OLD.flight_type, OLD.flight_status, NEW.flight_type, NEW.flight_status)
	INTO tot_passengers;
...
--TRIGGER QUANDO IL FLIGHT STATUS DI UN VOLO ARRIVING DIVENTA 'DEPARTED',
--SIMULIAMO UNA CONNESSIONE CON L'AEROPORTO DI PARTENZA: LE PRENOTAZIONI 
--'PENDING' DIVENTANO 'CANCELLED', E FARE IN MODO CHE VENGANO AGGIORNATI
--ANCHE I FREE_SEATS DI CONSEGUENZA CIRCA IL 90% DEI SUOI PASSEGGERI 
--CON PRENOTAZIONI CONFIRMED AVRANNO FATTO IL CHECK-IN, E TUTTI I 
--LUGGAGE_STATUS DEI BAGAGLI DEI SUOI PASSEGGERI CON CHECKED_IN A TRUE VENGONO MESSI A LOADED

CREATE OR REPLACE FUNCTION actual_func_simulate_connection_when_arriving_departed(
			   input_old_id_flight VARCHAR(15), input_old_flight_type BOOLEAN,
			   input_old_flight_status FlightStatus, input_new_flight_type BOOLEAN,
			   input_new_flight_status FlightStatus)
RETURNS INTEGER
AS $$
DECLARE
...
BEGIN
	...
	IF input_old_flight_type = false AND input_new_flight_type = false THEN
		...
		--questo if serve perché solo un volo PROGRAMMED o ABOUT_TO_DEPART
		--può essere impostato a DEPARTED
		IF input_old_flight_status <> 'PROGRAMMED' 
		   AND input_old_flight_status <> 'ABOUT_TO_DEPART' THEN

			RAISE EXCEPTION 'Il volo verso Napoli % non era in stato ''programmato'' o ''in
					partenza'', non può diventare ''decollato''!', OLD.id_flight;
		...
		IF input_old_flight_status <> 'DEPARTED' AND input_new_flight_status = 'DEPARTED' THEN
			...
			FOR selected_booking IN (SELECT * FROM BOOKING B
				...
				ELSE
				--basta else, perchè quelle già a CANCELLED non vengono prese dalla select
					FOR selected_ticket IN (SELECT * FROM TICKET T
								WHERE T.id_booking = selected_booking.id_booking) LOOP
						...
						IF (i % 10) <> 0 THEN
							...
							FOR selected_luggage IN (SELECT * FROM LUGGAGE L
								...
								UPDATE LUGGAGE
								SET luggage_status = 'LOADED',
								    id_luggage_after_check_in = selected_ticket.ticket_number || j
								WHERE id_luggage = selected_luggage.id_luggage;
...
BEGIN
	...
	SELECT actual_func_simulate_connection_when_arriving_departed(OLD.id_flight, OLD.flight_type,
	       OLD.flight_status, NEW.flight_type, NEW.flight_status)
	INTO tot_passengers;
...
BEGIN
	...
	IF NEW.flight_type = true THEN
		...
		IF NEW.flight_status = 'DEPARTED' OR NEW.flight_status = 'LANDED' THEN
		--non controllo se è 'ABOUT_TO_DEPART' perché se lo fosse, c'è un altro trigger
		--che aggiorna le 'PENDING' a 'CANCELLED'
		
			IF EXISTS(SELECT * FROM BOOKING B
				  WHERE B.id_flight = NEW.id_flight AND B.booking_status = 'PENDING') THEN

				RAISE EXCEPTION 'Il volo % non può partire finchè ha prenotazioni PENDING!',
						NEW.id_flight;
...
--TRIGGER UN VOLO ARRIVING ABOUT_TO_ARRIVE O LANDED NON HA PRENOTAZIONI PENDING non serve il controllo su 
--DEPARTED, perchè il trigger simulate_connection_when_arriving_departed le cancella BEFORE UPDATE, mentre,
--se viene inserito direttamente un volo come DEPARTED, allora non posso essere inserite prenotazioni PENDING
...
BEGIN
	...
	IF NEW.flight_type = false THEN
		...
		IF NEW.flight_status = 'ABOUT_TO_ARRIVE' OR NEW.flight_status = 'LANDED' THEN
			...
			IF EXISTS(SELECT * FROM BOOKING B
				  WHERE B.id_flight = NEW.id_flight
				  AND B.booking_status = 'PENDING') THEN

				RAISE EXCEPTION 'Il volo % non può partire finchè ha prenotazioni
						PENDING!', NEW.id_flight;
...
--Non serve invece il trigger per decremenrare i free_seats se una prenotazione passa da
--'CANCELLED' a qualcos'altro, perché non è possibile 'ripristinare' una prenotazione
...
BEGIN
	...
	IF NEW.flight_type = true THEN
		...
		IF NEW.id_gate IS NOT NULL THEN
			...
			IF NEW.flight_status <> 'ABOUT_TO_DEPART' AND NEW.flight_status <> 'DELAYED'
			   AND NEW.flight_status <> 'DEPARTED' AND NEW.flight_status <> 'LANDED' THEN

				RAISE EXCEPTION 'Il volo da Napoli % non è in stato ''in partenza'', ''in
						ritardo'', ''partito'', ''atterrato'', non lo si può inserire
						con un gate!', NEW.id_flight;
...
BEGIN
	...
	IF NEW.flight_type = false THEN
		...
		IF NEW.id_gate IS NOT NULL THEN
			...
			IF NEW.flight_status <> 'ABOUT_TO_ARRIVE' AND NEW.flight_status <> 'DELAYED'
			   AND NEW.flight_status <> 'DEPARTED' AND NEW.flight_status <> 'LANDED' THEN

				RAISE EXCEPTION 'Il volo per Napoli % non è in stato ''in arrivo'', ''in
						ritardo'', ''partito'', ''atterrato'', non lo si può inserire
						con un gate!', NEW.id_flight;
	...
	IF OLD.flight_type = true AND NEW.flight_type = true THEN
		...
		IF NEW.id_gate IS NOT NULL THEN
			...
			IF NEW.flight_status <> 'ABOUT_TO_DEPART' AND NEW.flight_status <> 'DELAYED' THEN
				...
				RAISE EXCEPTION 'Il volo da Napoli % non è in stato ''in partenza'' o ''in
						ritardo'', non gli si può assegnare un gate!', OLD.id_flight;
	...
	IF NEW.flight_type = false THEN
		...
		IF NEW.id_gate IS NOT NULL THEN
			...
			IF NEW.flight_status <> 'ABOUT_TO_ARRIVE' AND NEW.flight_status <> 'DELAYED' THEN
				...
				RAISE EXCEPTION 'Il volo verso Napoli % non è in stato ''in arrivo'' o ''in
						ritardo'', non gli si può assegnare un gate!', NEW.id_flight;
...
BEGIN
	...
	IF NEW.id_gate IS NOT NULL THEN
		...
		IF NEW.flight_status = 'PROGRAMMED' OR NEW.flight_status = 'CANCELLED' THEN
			...
			RAISE EXCEPTION 'Il volo % è in stato %, non gli si può assegnare un gate!',
					NEW.id_flight, NEW.flight_status;
	...
	IF NEW.flight_type = true THEN
		...
		IF NEW.id_gate IS NULL THEN
			...
			IF NEW.flight_status = 'DEPARTED' OR NEW.flight_status = 'LANDED' THEN
				...
				RAISE EXCEPTION 'Il volo da napoli % è in stato %, non può non avere un gate
						da cui è partito!', NEW.id_flight, NEW.flight_status;
	...
	IF NEW.flight_type = false THEN
		...
		IF NEW.id_gate IS NULL THEN
			...
			IF NEW.flight_status = 'LANDED' THEN
				...
				RAISE EXCEPTION 'Il volo verso napoli % è in stato ''atterrato'', non può
						non avere un gate dove è atterrato!', NEW.id_flight;
...
BEGIN
	...
	IF OLD.flight_type = true THEN
		...
		IF OLD.id_gate <> NEW.id_gate THEN
			...
			IF (OLD.flight_status = 'DEPARTED' OR OLD.flight_status = 'LANDED')
			   AND (NEW.flight_status = 'DEPARTED' OR NEW.flight_status = 'LANDED') THEN

				RAISE EXCEPTION 'Il volo da napoli % è in stato %, non si può modificare
						il gate da cui è partito!', OLD.id_flight, NEW.flight_status;
	...
	IF OLD.flight_type = false THEN
		...
		IF OLD.id_gate <> NEW.id_gate THEN
			...
			IF OLD.flight_status = 'LANDED' AND NEW.flight_status = 'LANDED' THEN
				...
				RAISE EXCEPTION 'Il volo per napoli % è in stato ''atterrato'', non si può
						modificare il gate in cui è atterrato!', OLD.id_flight;
...
--TRIGGER SE UN ACCOUNT CUSTOMER VIENE CANCELLATO, METTO A CANCELLED TUTTE
--LE SUE PRENOTAZIONI PER I VOLI CHE DEVONO ANCORA PARTIRE
...
--TRIGGER BLOCCARE L'INSERIMENTO DI QUALUNQUE PRENOTAZIONE SU UN VOLO IL CUI 
--FLIGHT_STATUS SIA ABOUT_TO_DEPART, DEPARTED, ABOUT_TO_ARRIVE O LANDED
...
BEGIN
	...
	IF associated_flight.flight_status = 'ABOUT_TO_DEPART'
	   ...
	   OR associated_flight.flight_status = 'LANDED' THEN
		...
		RAISE EXCEPTION 'Il volo % è in partenza oppure è già partito, non
				si possono inserire nuove prenotazioni!', NEW.id_flight;
...
--TRIGGER BLOCCARE L'INSERIMENTO DI QUALUNQUE PASSEGGERO SU UN VOLO IL CUI
--FLIGHT_STATUS SIA ABOUT_TO_DEPART, DEPARTED, ABOUT_TO_ARRIVE O LANDED
...
BEGIN
	...
	IF associated_flight.flight_status = 'ABOUT_TO_DEPART'
	   ...
	   OR associated_flight.flight_status = 'LANDED' THEN
		...
		RAISE EXCEPTION 'Il volo % è in partenza oppure è già partito, non
				si possono inserire biglietti!', NEW.id_flight;
...
--TRIGGER BLOCCARE L'INSERIMENTO DI QUALUNQUE BAGAGLIO PER UN PASSEGGERO DI UN VOLO 
--IL CUI FLIGHT_STATUS SIA ABOUT_TO_DEPART, DEPARTED, ABOUT_TO_ARRIVE O LANDED
...
BEGIN
	...
	IF associated_flight.flight_status = 'ABOUT_TO_DEPART'
	   ...
	   OR associated_flight.flight_status = 'LANDED' THEN
		...
		RAISE EXCEPTION 'Il volo % è in partenza oppure è già partito, il bagaglio
				non può essere inserito!', associated_flight.id_flight;
...
--TRIGGER NON SI POSSONO AGGIUNGERE NUOVE PRENOTAZIONI (BEFORE INSERT ON BOOKING)
--SE IL CUSTOMER ASSOCIATO HA IS_DELETED = TRUE
...
BEGIN
	...
	IF associated_customer.is_deleted = true THEN
		...
		RAISE EXCEPTION 'L''account dell''utente % è stato cancellato, non è
		possibile inserire nuove prenotazioni!', associated_customer.id_customer;
...
BEGIN
	...
	IF associated_flight.flight_status <> 'PROGRAMMED' THEN
		...
		RAISE EXCEPTION 'Il volo % non è in stato ''programmato'', non è possibile
				inserire nuove prenotazioni!', associated_flight.id_flight;
...
BEGIN
	...
	IF associated_flight.flight_status <> 'PROGRAMMED' THEN
		...
		RAISE EXCEPTION 'Il volo % non è in stato ''programmato'', non è possibile
		inserire nuovi biglietti!', associated_flight.id_flight;
...
BEGIN
	...
	IF associated_booking.booking_status = 'CANCELLED' THEN
		...
		RAISE EXCEPTION 'La prenotazione % è stata cancellata, non è possibile inserire
				nuovi biglietti!', associated_booking.id_booking;
...
BEGIN
	...
	IF associated_flight.flight_status <> 'PROGRAMMED' THEN
		...
		RAISE EXCEPTION 'Il volo % non è in stato ''programmato'', non è possibile
				inserire nuovi bagagli!', associated_flight.id_flight;
...
--e questo (insieme al trigger successivo) garantisce anche non possano essere inseriti
--bagagli per voli già partiti
...
BEGIN
	...
	IF associated_ticket.checked_in = true THEN
		...
		RAISE EXCEPTION 'Per il biglietto con numero % è già stato fatto il check-in,
				non è possibile inserire nuovi bagagli!',
				associated_ticket.ticket_number;
...
--e questo (insieme al trigger precedente) garantisce anche non possano essere
--inseriti bagagli per voli già partiti
...
BEGIN
	...
	IF associated_booking.booking_status = 'CANCELLED' THEN
		...
		RAISE EXCEPTION 'La prenotazione % è stata cancellata, non è possibile inserire
				nuovi bagagli!', associated_booking.id_booking;
...
BEGIN
	...
	IF OLD.flight_type = false THEN 
		...
		IF NEW.flight_status = 'DELAYED' OR NEW.flight_status = 'ABOUT_TO_ARRIVE'
		   OR NEW.flight_status = 'LANDED' THEN

			IF OLD.free_seats <> NEW.free_seats THEN

				RAISE EXCEPTION 'Il volo % è già partito, non si possono
						modificare i suoi posti liberi!', OLD.id_flight;
	...
	IF OLD.flight_type = true THEN 
		...
		IF NEW.flight_status = 'DEPARTED' OR NEW.flight_status = 'LANDED' THEN
			...
			IF OLD.departure_time <> NEW.departure_time OR OLD.free_seats <> NEW.free_seats THEN
				...
				RAISE EXCEPTION 'Il volo % è già partito, non si possono modificare
				i suoi: orario di partenza, posti liberi!', OLD.id_flight;
...
BEGIN
	...
	IF OLD.booking_status = 'CANCELLED' THEN
		...
		RAISE EXCEPTION 'La prenotazione % è stata cancellata, non può essere modificata!',
				OLD.id_booking;
...
BEGIN
	...
	IF OLD.flight_status = 'CANCELLED' THEN
		...
		RAISE EXCEPTION 'Il volo % è stata cancellata, non può essere modificata!',
				OLD.id_flight;
